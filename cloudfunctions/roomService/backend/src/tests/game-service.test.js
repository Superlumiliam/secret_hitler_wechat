...
test("游戏命令会处理版本冲突和幂等重试", async () => {
  const { call, roomId, openids } = await setupStartedGame();

  const p1Snapshot = await call(openids[0], "getGameSnapshot", { roomId });
  const p2Snapshot = await call(openids[1], "getGameSnapshot", { roomId });

  const firstAck = await call(openids[0], "submitCommand", {
    roomId,
    commandId: "same-ack",
    expectedVersion: p1Snapshot.version,
    taskId: p1Snapshot.pendingTask.taskId,
    type: "ACK_ROLE_REVEAL",
    body: { acknowledged: true }
  });
  assert.equal(firstAck.deduplicated, false);

  const deduplicatedAck = await call(openids[0], "submitCommand", {
    roomId,
    commandId: "same-ack",
    expectedVersion: p1Snapshot.version,
    taskId: p1Snapshot.pendingTask.taskId,
  ...
  assert.equal(deduplicatedAck.deduplicated, true);

  await assert.rejects(
    call(openids[1], "submitCommand", {
      roomId,
      commandId: "stale-ack",
      expectedVersion: p2Snapshot.version,
      taskId: p2Snapshot.pendingTask.taskId,
      type: "ACK_ROLE_REVEAL",
      body: { acknowledged: true }
    }),
    (error) => error.result.error.code === "VERSION_CONFLICT"
  );
});
