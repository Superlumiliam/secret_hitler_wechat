...
只有两类“终局相关的独裁者信息”必须说真话：

- 玩家被处决时，如果他是独裁者，必须如实确认。
- 当已颁布至少 3 张极权派政策后，如果新当选总理是独裁者，必须如实确认。

## 2. 人数配置

### 2.1 身份分配表

| 玩家数 | 自由派 | 普通极权派 | 独裁者 | 独裁者是否知道普通极权派 |
| --- | --- | --- | --- | --- |
| 5 | 3 | 1 | 1 | 是 |
| 6 | 4 | 1 | 1 | 是 |
| 7 | 4 | 2 | 1 | 否 |
...
- 普通极权派始终知道彼此是谁，也知道谁是独裁者。
- 只有在 `5-6` 人局时，独裁者会知道普通极权派是谁。
- 在 `7-10` 人局时，独裁者不知道普通极权派是谁。
...
1. 根据玩家人数选择对应的极权派政策轨。
2. 放置一条固定的自由派政策轨。
3. 将 `11` 张极权派政策牌与 `6` 张自由派政策牌混洗为一个牌库。
4. 准备弃牌堆。
5. 将选举计数器标记放在起始位置。
...
- 独裁者的秘密身份是 `Hitler`
- 但独裁者的党派归属卡显示为 `Fascist`

这样设计是为了保证“调查忠诚”只会看到阵营，不会直接暴露某人是不是独裁者。
...
- 不是每轮都会有执行阶段。
- 只有当本轮颁布的是“带总统权力的极权派政策”时，才会进入执行阶段。
...
flowchart TD
  ...
  E -->|达到3| F["翻开牌库顶牌并直接颁布"]
  F --> G["忽略该政策对应的总统权力"]
  G --> H["重置选举计数器，并清空当前总理任期限制"]
  H --> A
  D -->|通过| I["政府成立"]
  I --> J{"当前极权派政策数 >= 3 且新总理是独裁者?"}
  J -->|是| K["极权派立即获胜"]
  J -->|否| L["进入立法阶段"]
  L --> M["总统摸3弃1，总理2选1颁布"]
  M --> N{"本次颁布政策是否触发总统权力?"}
...
总统候选人可以在提名前后和全场讨论总理人选，以争取大家对候选政府形成共识。
...
投票前，玩家可以讨论这组候选政府是否可信、是否应该通过。这里讨论的是“是否让这组政府上台”，不是立法后的牌面复盘。
...
若此时场上已经颁布了至少 `3` 张极权派政策，则需要立刻检查：

- 如果新当选总理是独裁者，极权派立即获胜，游戏结束。
- 如果新当选总理不是独裁者，则游戏继续进入立法阶段。

这个规则意味着：

- 当一名玩家在 `3+` 极权派政策后成功当选总理且游戏没有结束时，全场都能确定他“不是独裁者”。
...
立法结束、政策正面颁布后，玩家可以讨论本轮立法结果。总统和总理可以解释自己看到了什么、弃了什么，也可以对这些隐藏信息说谎；其他玩家也可以围绕牌面说法、投票动机和总统权力目标进行质询。
...
- 若极权派政策累计达到 `6`，极权派立即获胜。
- 若本次颁布的是带总统权力的极权派政策，则进入执行阶段。
- 其他情况直接进入下一轮选举阶段。

## 7. 执行阶段与总统权力

只有当本轮颁布的极权派政策格子上带有总统权力时，当前总统才必须执行一次权力。  
该权力必须在下一轮开始前完成。

执行权力前，总统可以和其他玩家讨论目标或选择；但最终由总统决定如何、何时使用这次权力。游戏不能在总统权力完成前进入下一轮。

### 7.1 不同人数下的极权派政策轨权力表

| 已颁布极权派政策数 | 5-6 人局 | 7-8 人局 | 9-10 人局 |
| --- | --- | --- | --- |
| 1 | 无 | 无 | 调查忠诚 |
| 2 | 无 | 调查忠诚 | 调查忠诚 |
| 3 | 政策预览 | 特别选举 | 特别选举 |
...
- 第 `5` 张极权派政策颁布后，从之后的立法阶段开始，会额外永久解锁“否决权”。
- 第 `6` 张极权派政策不是权力格，而是极权派直接胜利。
...
- 总统看到的只有阵营归属，自由派看到的是 `Liberal`，极权派与独裁者看到的都是 `Fascist`。
- 总统可以公开说明结果，也可以撒谎。
- 总统不能调查自己。
- 同一名玩家整局游戏不能被重复调查两次。
...
- 如果被处决者是独裁者，自由派立即获胜。
- 如果被处决者不是独裁者，场上不公开他的真实身份或阵营。
- 总统可以选择处决自己；此时按普通处决流程结算。
- 被处决玩家立即出局，且之后不能：
  - 发言
  - 投票
  - 被提名
...
当第 `5` 张极权派政策已经被颁布后，从之后每一次立法阶段开始，否决权都可用。

注意：

- 第 `5` 张极权派政策本身照常先触发该格对应的“处决”。
...
每一次成功否决都会让选举计数器前进 `1` 格。

因此：

- 否决可能导致选举计数器到达 `3`。
- 如果因此达到 `3`，则按“三连败与混乱政策”的规则处理：翻开牌库顶牌直接颁布，忽略权力，重置选举计数器。
...
调查忠诚不会直接分辨“普通极权派”和“独裁者”。  
两者在党派归属卡上都显示为 `Fascist`。

### 9.7 非独裁者在 3 张极权派政策后成功当选总理，会得到公开确认

如果游戏没有因为“独裁者当选总理”而结束，那么所有人都能推出：
...
处决只在“击中独裁者”时公开真实结果。  
如果被处决者不是独裁者，其阵营依旧是隐藏信息。

### 9.9 特别选举不会改变长期轮换顺序

特别选举只是临时插入一次总统候选人，不会把轮换顺序永久改写。

### 9.10 第 5 张极权派政策既会处决，也会为后续回合解锁否决权
...
1. 提名时先判断总理候选人是否符合资格。
2. 投票结算后，若通过且当前极权派政策数已达 `3+`，先检查总理是否是独裁者。
3. 若未结束，再进入立法阶段。
4. 政策颁布后，先检查是否达到 `5` 自由派或 `6` 极权派政策的胜利条件。
5. 若未结束，再判断本次极权派政策是否赋予总统权力。
...
- 当前是否因为 `3+` 极权派政策而进入“独裁者当选即输”的危险阶段
