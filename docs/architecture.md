...
| `room_members` | 房间成员类型、类型内席位、准备态、在线态和虚拟玩家归属 | 服务端 |
| `user_profiles` | 以 openid 为主键的用户资料、会话恢复锚点和多人对局聚合统计 | 服务端 |
...
大厅没有公共或私密快照文档；`getLobbySnapshot` 从 `rooms + room_members` 即时构建视图。`room_members.memberType` 区分 `player` 与 `spectator`，历史记录缺失时按玩家解释；两类席位分别编号，`rooms.playerCount` 只统计玩家。开局后，玩家名单冻结，`game_core` 是真相源，每次成功状态迁移同步更新核心状态、公共投影、受影响的私密投影、事件和同步信号。投票未揭示时，`game_core.phaseData.votesByMemberId` 保留逐玩家票型和命令元数据；全部存活玩家提交后，公开结果、公开历史和结果时间线只保存按席位排序的 `voteGroups.jaMemberIds / neinMemberIds`。`game_core.legislativeHistory` 只保存结果复盘所需的总统弃牌、总理弃牌和总理颁布牌事实，不保存双方看到的牌或其他可推导字段；总统弃牌、总理颁布和总统同意否决与对应状态迁移在同一事务内写入。私密投影在事务内使用迁移前后的 `game_core` 在内存中比较，只写入身份、投票、手牌、调查结果、政策预览、待办任务或终局状态实际变化的玩家席，不读取 `player_private_snapshots` 做比较；开局初始化也只为玩家创建私密文档。

## 状态与命令处理

房间状态为 `lobby → in_game → ended`，过期后进入 `expired` 并由维护链路清理。

大厅加入在事务中优先占用玩家席，玩家坐满后占用固定 3 个观战席；跨类型换席只允许在大厅发生。进入 `in_game` 后不再增加或替换玩家，只允许新用户占用空观战席，离线玩家重新加入则恢复原玩家记录。
...
- 公共快照只含所有玩家都能看到的信息，包括座位、政府、政策轨、按赞同/反对分组的公开票型和公共历史；历史轮次公开投影同时记录总理颁布的政策、总统完成的权力和否决结果；进行中的快照不包含 `legislativeHistory`。服务端公共快照文档当前同时保留面向投影构建的原始 `publicHistory` 和面向客户端的 `history`，但 `getGameSnapshot` 响应只下发 `history`，不下发 `publicHistory`。
...
客户端只消费版本不小于本地版本的快照。公共快照版本是客户端权威版本；局内观战加入或离开不推进游戏版本，同步信号必须使用公共投影版本，仅在公共快照缺失时回退房间版本，不能把独立的大厅/房间版本混入游戏版本序列。未发生私密投影变化的私密文档可以保留上一次私密写入的 `version` 和 `updatedAt`，因为 `getGameSnapshot` 使用公共投影提供当前版本。页面隐藏或卸载时关闭监听和轮询，避免后台空耗。快照缓存用于页面间快速呈现，但缓存不能替代服务端刷新和权限校验。

活跃房间恢复优先使用 `user_profiles` 中的锚点；有效的大厅、对局和结果锚点分别恢复到对应页面。开局时为全部真实成员更新锚点，玩家和观战者均可恢复。资料存在但锚点缺失或失效时，服务端只从真实成员仍为 `active`、房间未过期且状态为 `lobby` 或 `in_game` 的成员记录中自愈，并且只有恰好一个合法候选才在事务重检后写回。`offline`、虚拟成员、结束或过期房间以及多候选均不自愈，防止撤销主动退出或错误选择历史房间。
...
- 自动化测试覆盖规则场景、开局、结果快照、房间隔离与并发、玩家/观战席争抢、观战权限、恢复、维护并发、幂等记录、同步降级、缓存和前端映射。
