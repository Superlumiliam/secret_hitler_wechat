...
大厅没有公共或私密快照文档；`getLobbySnapshot` 从 `rooms + room_members` 即时构建视图。开局后，`game_core` 是真相源，每次成功状态迁移同步更新核心状态、公共投影、受影响的私密投影、事件和同步信号。私密投影在事务内使用迁移前后的 `game_core` 在内存中比较，只写入身份、投票、手牌、调查结果、政策预览、待办任务或终局状态实际变化的席位，不读取 `player_private_snapshots` 做比较；开局初始化仍为全部成员创建私密文档。
...
客户端只消费版本不小于本地版本的快照。公共快照版本是客户端权威版本；未发生私密投影变化的私密文档可以保留上一次私密写入的 `version` 和 `updatedAt`，因为 `getGameSnapshot` 使用公共投影提供当前版本。页面隐藏或卸载时关闭监听和轮询，避免后台空耗。快照缓存用于页面间快速呈现，但缓存不能替代服务端刷新和权限校验。
