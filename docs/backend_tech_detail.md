...
1. MVP 不开放前端直接订阅数据库快照，默认通过云函数轮询读取快照。
2. 核心真相只保存在后端内部集合，前端永远不拿完整真相。
3. 每次成功写入后同步重建公共快照和全部私密快照，优先保证一致性，暂不优先优化写放大。
4. 现有 `cloudfunctions/quickstartFunctions` 仅视为模板示例，不继续在其上堆业务代码。
5. 游戏内所有写操作统一走 `gameService.submitCommand`，大厅阶段写操作统一走 `roomService`。

### 2.3 为什么默认不用数据库直订阅

虽然 `/docs/front_back_api.md` 允许快照订阅，但本项目存在两类高敏信息：

- 仅当前玩家可见的私密快照
- 仅系统可见的完整真相

MVP 阶段为了降低泄露风险和权限配置复杂度，采用：
...
1. 不允许继续把业务逻辑塞进 `quickstartFunctions`
2. 不允许前端直接写任何核心集合
3. 不允许在云函数入口文件中直接写规则判断
4. 不允许跳过事件日志只维护最终快照
5. 不允许只更新公共快照、不更新私密快照
6. 不允许为了“减少一次查询”而把完整真相下发给前端
