...
监听连接失败或断开时，客户端按 `30000ms`、`60000ms`、`120000ms`、`300000ms` 的顺序退避重建 watcher；达到最大间隔后每 `300000ms` 重试。成功收到有效信号后重置退避序列，页面隐藏或卸载时清除重建定时器。
...
- watcher 重建期间继续执行监听异常时的轮询降级；watcher 重建只恢复实时信号通知，不替代快照读取
