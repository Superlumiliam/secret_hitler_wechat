...
5. 前端只能只读监听当前活跃房间的 `room_sync_signals`；不能写入信号，也不能直接读写或监听其他后端集合。
...
采用“只读实时信号 + 云函数快照 + 轮询降级”的混合方案。前端只监听不含游戏真相的 `room_sync_signals`，版本变化后仍通过云函数 action 获取完整大厅视图或对局快照；公共、私密和核心状态集合继续禁止前端读取。
...
2. 页面可见时监听房间同步信号；信号变化后拉取大厅视图
3. 监听健康时每 `10000ms` 校准一次；监听异常时立即恢复 `3000ms` 轮询
4. 监听连接失败或断开后，按 `30000ms`、`60000ms`、`120000ms`、`300000ms` 退避重建；达到最大间隔后每 `300000ms` 重试，成功收到有效信号后重置退避序列
5. 页面隐藏时关闭监听、清除重建定时器并停止轮询
6. 命令提交成功后使用返回视图或立即补拉最新大厅视图

轮询间隔建议：

- 实时监听健康：`10000ms` 一致性校准
- 实时监听异常：`3000ms` 降级轮询
- 页面隐藏：停止轮询
...
2. 桌面页可见时监听房间同步信号；在同一房间状态内记录最高待处理版本，房间状态变化必须独立触发快照，不受大厅版本与对局版本重新从 `1` 开始的影响；请求结束时若仍落后则补拉，且不得并发请求
3. 监听健康时每 `60000ms` 校准一次；监听异常时恢复原动态轮询
4. 监听连接失败或断开后，按 `30000ms`、`60000ms`、`120000ms`、`300000ms` 退避重建；达到最大间隔后每 `300000ms` 重试，成功收到有效信号后重置退避序列
5. 桌面页隐藏时关闭监听、清除重建定时器并停止轮询
6. 身份页优先使用当前 `gameStore.privateState.identity`；缺失时只补拉一次 `getGameSnapshot`，不启动独立监听
7. 命令提交成功、版本冲突或阶段变化时立即补拉一次最新快照

监听重建只负责恢复 `room_sync_signals` 的只读通知连接；重建期间仍按监听异常策略执行快照轮询，不影响页面数据兜底。

轮询间隔建议：

- 实时监听健康：`60000ms` 一致性校准
- 实时监听异常且本人有待办：`1000ms`
- 实时监听异常且本人无待办：`4000ms`
- 实时监听异常且刚提交命令：`800ms`
- 页面隐藏：停止轮询
...
1. 用户在对局桌面页点击底部“我的身份 / 查看身份”入口，或点击当前用户自己的席位头像，参考 `reference/04-对局桌面页.png`。
...
1. 公共桌面始终可见。
2. 页面提供“我的身份 / 查看身份”入口；当前用户自己的席位头像也是身份页快捷入口，二者点击后跳转同一身份页。
...
1. 对局桌面页提供“我的身份 / 查看身份”入口，并允许点击当前用户自己的席位头像快捷进入身份页。
...
2. 桌面页点击“我的身份 / 查看身份”或自己的席位头像 -> 身份页点击“我知道了” -> 返回桌面
3. 提名 -> 投票通过 / 失败
4. 三连败自动翻牌
5. 总统弃牌 -> 总理立法
6. 第 5 张极权派政策后的否决流程
...
4. 数据同步采用“只读实时信号通知 + 云函数读取快照 + 轮询降级”。
